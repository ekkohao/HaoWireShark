...
namespace tcpip {
    ...
    */
    ...
    inline u_short host2net(u_short h);
    // 模拟htons函数，本机字节序转网络字节序,long
    inline u_short host2netl(u_long h);

    // 模拟ntohl函数，网络字节序转本机字节序
    inline u_long net2host(u_short n);
    // 模拟ntohl函数，网络字节序转本机字节序
    inline u_long net2hostl(u_long n);
    // 将字节序ip地址转为点分十进制的字符串地址,并获取
    inline char *ipchar2normal(u_long in);
    // 将点分十进制的字符串地址转网络字节整形
    inline u_int ipnormal2net(const char *ptr);
    ...
    /** 将字节序ip地址转为点分十进制的字符串地址 */
    #define IPTOSBUFFERS    12
    char *ipchar2normal(u_long in)
    {
    ...
    u_int ipnormal2net(const char *ptr)
    {
        int a[4],i=0;
        char str[255] = {0};
        u_long num;
    ...
    /**  */
    inline char hexStr2char(QString s);
    inline int hexChar2Int(char c);

    char hexStr2char(QString s) {
        if(s.length() != 2)
            return 0;
        char c,c1,c2;
        c1 = s.at(0).toLower().toLatin1();
        c2 = s.at(1).toLower().toLatin1();
        c = hexChar2Int(c1) * 16 + hexChar2Int(c2);
        QChar qc(c);
        if( qc.isPrint())
            return c;
        else
